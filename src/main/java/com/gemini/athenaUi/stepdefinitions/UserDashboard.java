    ...
    public void enterCourseFields(String courseType,String duration,String fileLocation,String category)
{
    try
    {
        DriverAction.waitUntilElementClickable(UserDashboard_Locator.addContent,10);
        int c=2;
...
//in this function we are weather assigned learner is visible on user dashboard
            ...
            if(DriverAction.isExist(UserDashboard_Locator.learnerId))
            {
               ...
               else
               {
                ...
                GemTestReporter.addTestStep("Error Occur", "Fail to find learner's id" , Status.FAIL,
                        DriverAction.takeSnapShot());
            }
...
if(DriverAction.isExist(UserDashboard_Locator.courseFilterInput))
{
    ...
    try{
        ...
        if (DriverAction.isDisplayed(Course_Locators.editIcon)) {
            String assignedCourseName=DriverAction.getElementText(Course_Locators.draftedCourse);
            DriverAction.click(Course_Locators.editIcon, "clicked on Dropdown icon", "Successfully clicked");
            if(DriverAction.isDisplayed(By.xpath(Course_Locators.adminOption.replace("input","Edit Course"))));
            {
                DriverAction.click(By.xpath(Course_Locators.adminOption.replace("input","Edit Course")));
            }
        }
        if(DriverAction.isDisplayed(UserDashboard_Locator.confirmationDialog)){
            DriverAction.click(By.xpath(MyLocators.button.replace("input","No")));
        }
        DriverAction.waitSec(3);
        ...
        DriverAction.scrollToBottom();
        if(DriverAction.isDisplayed(UserDashboard_Locator.addContentBtn))
        {
    ...
    public void validateUpdateCourseDashboard()
    {
        try{
            ...
            if(DriverAction.isExist(UserDashboard_Locator.courseName))
            {
                ...
                {
                    GemTestReporter.addTestStep("Validate Course Updated is present on User Dashboard", "Course is not present on User Dashboard" , Status.FAIL,
                            DriverAction.takeSnapShot());
                }
    ...
    public void validateDeleteCourseDashboard()
    {
        try{
            ...
            if(DriverAction.isExist(UserDashboard_Locator.courseName))
            {
                ...
                if(_CourseName.equals(getCourse))
                {
                    GemTestReporter.addTestStep("Validate Course Deleted on User Dashboard", "Course is present on User Dashboard" , Status.FAIL,
                            DriverAction.takeSnapShot());
                }
                else
                {
                    GemTestReporter.addTestStep("Validate Course Deleted on User Dashboard", "Course is not present on User Dashboard" , Status.PASS,
                            DriverAction.takeSnapShot());
                }
    ...
    public void addAnAssignment() {
    try{
        DriverAction.click(UserDashboard_Locator.addAssignment);
    }catch(Exception e){
        GemTestReporter.addTestStep("Add an assignment","Exception encountered- "+e,Status.ERR);
    }
    }
    ...
    try {
        DriverAction.typeText(UserDashboard_Locator.searchCourse, _CourseName,"Search course name- "+_CourseName);
    }catch(Exception e){
        GemTestReporter.addTestStep("Search course name","Exception encountered- "+e,Status.ERR,DriverAction.takeSnapShot());
    }
    }
