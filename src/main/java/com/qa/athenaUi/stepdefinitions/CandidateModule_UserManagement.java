...
public class CandidateModule_UserManagement {
    ...
    public void login(String username, String password) {
        ...
        try {
            ...
            DriverAction.waitUntilElementClickable(MyLocators.signInBtn, 45);
            DriverAction.click(MyLocators.signInBtn);
            DriverAction.waitUntilElementClickable(MyLocators.sidebar, 45);
            //   Thread.sleep(25000);
//            changeResolution1();
        } catch (Exception e) {
    ...
    public void selectFromSidebar(String module, String submodule) {
        try {
            Thread.sleep(3000);
            DriverAction.waitUntilElementClickable(MyLocators.sidebar, 10);
...
//            Date time1 = new Date(); // Capture the first time
//            // Perform some operations here
//
//            Date time2 = new Date(); // Capture the second time
//
//            long differenceInMilliseconds = time2.getTime() - time1.getTime();
//
//            // Convert milliseconds to other units (optional)
...
//
//            System.out.println("Time difference in milliseconds: " + differenceInMilliseconds);
...
//            System.out.println("Time difference in hours: " + differenceInHours);
...
//    }
    ...
    public void selectRolesIcon() {
        ...
        try {
            DriverAction.waitUntilElementClickable(MyLocators.navbarRolesDropdown, 5);
    ...
    public void selectActionsIconEmployees() {
        try {
            DriverAction.waitUntilElementClickable(MyLocators.employeeActionsIcon, 7);
    ...
    public void editProfile() {
        try {
            DriverAction.waitUntilElementClickable(MyLocators.editProfile, 5);
    ...
    public void selectFromDropdown(String option) {
        try {
            ...
            } else if (DriverAction.isDisplayed(By.xpath(QuestionsLocators.codingLanguageOptions.replace("input", option)))) {
                DriverAction.click(By.xpath(QuestionsLocators.codingLanguageOptions.replace("input", option)), "Select " + option + " from dropdown", "Successfully selected " + option + " from dropdown.");
            } else if (DriverAction.isDisplayed(By.xpath(Course_Locators.selectFileType.replace("input", option)))) {
                DriverAction.click(By.xpath(Course_Locators.selectFileType.replace("input", option)), "Select " + option + " from dropdown", "Successfully selected " + option + " from dropdown.");
            } else {
    ...
    public void pageNavigate(String page) {
        try {
            Thread.sleep(15000);
            //    DriverAction.waitUntilElementClickable(By.xpath(MyLocators.pageNavigate.replace("input", page)));
...
//                }
    ...
    public void verifyAnswers(String questionStatus) {
        try {
            Thread.sleep(4000);
            DriverAction.waitSec(4);
            String Status_check = DriverAction.getAttributeName(MyLocators.paletteBtn, "class");

            DriverAction.waitSec(3);
            Thread.sleep(5000);

            //if answer selected is saved
            if (Status_check.contains(questionStatus)) {
...
//                    }
    ...
    public void validateTheModulesPresent() {
        try {
            DriverAction.waitSec(2);
            String[] modules = new String[]{"User Dashboard", "Test Analytics", "Tests", "Manage Courses", "Mail Configurations", "Manage Sections", "Reports", "Campus", "User Management", "Role Management", "Feedbacks"};
            String getModule;
            int numModule = 0;
    ...
    public void validateTheSubmodulesPresent() {
        try {
            DriverAction.waitSec(3);
            String[] submodules = new String[]{"Test Control", "Questions", "Course Library", "Content Library", "Assignment Library", "Batches", "Template Library", "Send Custom Mail"};
    ...
    public void clickViewReportInAttemptedTests() {
        try {
            DriverAction.waitUntilElementClickable(MyLocators.viewReportInAttemptedTest, 10);
    ...
    @And("Expand dropdown in user management {string}")
    public void expandDropdownInUserManagement(String dropdown) {
        try {
            DriverAction.click(By.xpath(MyLocators.dropdown.replace("input", dropdown)), "Expand dropdown in user management- " + dropdown, "Successfully expanded the dropdown.");
        } catch (Exception e) {
            GemTestReporter.addTestStep("Expand dropdown in user management- " + dropdown, "Exception encountered- " + e, Status.ERR, DriverAction.takeSnapShot());
        }
    }

    @Given("Get current time")
    public void getCurrentTime() {
        LocalDateTime now = LocalDateTime.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("HH:mm:ss");
        String formattedTime = now.format(formatter);
        System.out.println("Current time in specific format: " + formattedTime);
    }

    @And("^Search a user \"([^\"]*)\"$")
    public void searchAUser(String user) {
        try {
